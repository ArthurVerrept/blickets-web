...
async function getMyTickets() {
  ...
  for (const ticket of res.events) {
    const jsonMetadataRes = await axios.get(ticket.tokenURI)
    returnArray.events.push({...ticket, imageURL: jsonMetadataRes.data.image })
  }
      ...
      <div class="flex justify-evenly">
        ...
        <div class="block px-6 pb-6 w-1/2 bg-white border border-gray-200 h-[40rem] overflow-auto">
          ...
          <div v-if="!myTicketsLoading && myTickets.events" v-for="ticket in myTickets.events" class="my-2">
            <div class="relative flex mb-[-170px] hover:mb-0 transition-all">
              <img class="w-full object-cover h-72 rounded-2xl" :src="ticket.imageURL" alt="">
              <div class="absolute p-5 h-full w-full">
                ...
                <!-- {{ticket}} -->
                <!-- <div class="relative">
                    <p class="absolute text-4xl font-extrabold text-black">{{ticket.eventName}}</p>
                    <p class="absolute text-4xl font-extrabold text-white">{{ticket.eventName}}</p>
                  </div> -->
